...
divisor:: Int -> [Int]
divisor 1 =[1]
divisor x =1 :[ a|a <-[2..(x`div`2)],x `mod` a == 0]++[x]

okPrime :: Int->Bool
okPrime x = divisor x ==[1,x]

primes :: Int -> [Int]
primes n = [x|x<-[2..n],okPrime x]
...
-}
nrOcc :: Ord a => [a] -> [(a, Int)]
aux = \list -> ((head list), (length list))
nrOcc = map (aux) . group2 . sort
...
dup :: String -> String
dup = unwords . map (\w -> (duplicate w 1) ++" "++ (duplicate w 1)  ) . words
