...
arithmetic first q = iter first q
              where iter first q = first : iter (first + q) q
geometric :: Num a => a -> a -> [a]
geometric  first q = iter first q
            where iter first q = first : iter (first * q) q
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms seed = convert ( next ( mkStdGen seed ) )
convert (x , generator) = (fromIntegral x ) : (convert ( next generator ) )
{-
    4. (1p)
...
tableToFunc table key = crypted
  where (_, crypted) : _ = filter (aux key) table
